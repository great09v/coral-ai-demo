...
def get_patches_from_image(im, coords_index, patch_size, discard_cropped=False):
    ...
    for (r, c), p in patches_bounds.iterrows():
        patches.append(im[int(p.min_r): int(p.max_r) + 1, int(p.min_c): int(p.max_c) + 1])
...
def write_patches_as_images2(image_name, patches, out_dir):
    image_name = image_name.split(".")[0]
    patches = pd.DataFrame(patches)

    for ind, patch in patches.iterrows():
        image_path = os.path.join(out_dir, image_name)

        if not os.path.exists(image_path):
            os.mkdir(image_path)

        file_name = '{}.png'.format(ind)
        cv2.imwrite(os.path.join(image_path, file_name), patch, [cv2.IMWRITE_PNG_COMPRESSION, 9])